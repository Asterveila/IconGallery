fetch("assets/iconkit-data.json")
  ...
  .catch(() => {
    console.warn("assets/iconkit-data.json not found");
  });
...
function loadSettings() {
  ...
  const savedFilename =
    localStorage.getItem("gdGalleryFilename") || "{iconName}-({iconFilenames})";
  if (saved) {
...
function setupControls() {
  ...
  document.getElementById("btnInfoFilename").addEventListener("click", () => {
    document.getElementById("filenameInfoPopup").classList.add("active");
  });
  document
    .getElementById("btnCloseFilenameInfo")
    .addEventListener("click", () => {
  ...
  function updateHighlight() {
    ...
    fnHigh.innerHTML = safeVal.replace(
      /({iconName}|{iconAuthor}|{iconFilenames}|{gamemode}|{format})/g,
      '<span style="color: #ffee00;">$1</span>',
    );
  }

  fnInput.addEventListener("input", updateHighlight);
  fnInput.addEventListener(
    "scroll",
    () => (fnHigh.scrollLeft = fnInput.scrollLeft),
  );
  updateHighlight();
...
async function parseGdicon(blob, sourcePath = "") {
  ...
  if (previewEntry) {
    previewBlob = new File([await previewEntry.async("blob")], "preview.png", {
      type: "image/png",
    });
  }
  ...
  async function getPlistFile() {
    if (!_plistFile) {
      _plistFile = new File([await plistEntry.async("blob")], plistEntry.name, {
        type: "text/xml",
      });
    }
    return _plistFile;
...
function applyFilters() {
  ...
  filteredIcons = allIcons.filter((icon) => {
    ...
    if (search) {
      const hay = [m.iconName, m.author, m.description, ...(m.collabWith || [])]
        .join(" ")
        .toLowerCase();
...
function buildCard(icon) {
  ...
  card
    ...
    .addEventListener("click", () => downloadIcon(icon));
  if (m.hasProjectFiles) {
    card
      .querySelector(".project-btn")
      .addEventListener("click", () => downloadProjectFiles(icon));
  }
  return card;
}
...
async function setupIconImage(icon, card) {
  ...
  try {
    ...
    await renderer.renderIcon(pngFile, plistFile, icon.meta, getColorOpts());
...
function packRectangles(frames, gap = 2) {
  ...
  for (const frame of sortedFrames) {
    ...
    if (!placed) {
      const newY =
        bins.length > 0
          ? Math.max(...bins.map((bin) => bin.y + bin.maxHeight + gap))
          : 0;
      const newBin = {
        x: 0,
        y: newY,
  ...
  const sheetHeight =
    bins.length > 0 ? Math.max(...bins.map((bin) => bin.y + bin.maxHeight)) : 0;
