...
function setupControls() {
    ...
    document.getElementById('btnInfoBasicPreview').addEventListener('click', () => {
    ...
    document.getElementById('basicPreviewCheck').addEventListener('change', e => {
    ...
    document.getElementById('btnNextPage').addEventListener('click', () => {
        ...
        if (currentPage < maxPages) { currentPage++; renderGallery(); }
    });

    // goto page
    document.getElementById('btnOpenPagePopup').addEventListener('click', () => {
    ...
    document.getElementById('overrideColors').addEventListener('change', e => {
    ...
    for (const id of ['overrideCol1', 'overrideCol2', 'overrideGlow']) {
        document.getElementById(id).addEventListener('input', e => {
            const swatchIds = { overrideCol1: 'swatchCol1', overrideCol2: 'swatchCol2', overrideGlow: 'swatchGlow' };
            document.getElementById(swatchIds[id]).style.background = e.target.value;
        });
    }
...
async function loadIconRegistry() {
    ...
    try {
        ...
        } catch (e) { console.warn("Could not fetch downloads", e); }

        const res = await fetch('icons/registry.json');
        if (!res.ok) throw new Error(`registry.json responded with HTTP ${res.status}`);
        ...
        results.forEach((r, i) => {
            if (r.status === 'rejected') console.warn(`Failed to load ${registry[i]}:`, r.reason);
        });
...
async function parseGdicon(blob, sourcePath = '') {
    ...
    if (!pngEntry || !plistEntry) throw new Error('Missing PNG or PLIST inside ' + sourcePath);
    ...
    if (previewEntry) {
        previewBlob = new File([await previewEntry.async('blob')], 'preview.png', { type: 'image/png' });
    }

    return { id: sourcePath, meta, pngFile: pngBlob, plistFile: plistBlob, previewFile: previewBlob, sourcePath };
}
...
function applyFilters() {
    ...
    filteredIcons = allIcons.filter(icon => {
        const m = icon.meta;
        if (activeGamemode !== 'all' && m.iconType !== activeGamemode) return false;
        ...
        if (search) {
            const hay = [m.iconName, m.author, m.description, ...(m.collabWith || [])].join(' ').toLowerCase();
    ...
    filteredIcons.sort((a, b) => {
        switch (order) {
            ...
            case 'name-asc':   return a.meta.iconName.localeCompare(b.meta.iconName);
            case 'name-desc':  return b.meta.iconName.localeCompare(a.meta.iconName);
            case 'author-asc': return a.meta.author.localeCompare(b.meta.author);
            default: return 0;
        }
    });
    currentPage = 1;
...
async function renderGallery() {
    ...
    if (totalFiltered === 0) {
        ...
        document.getElementById('statsLabel').textContent = `Showing 0 through 0 out of 0`;
    ...
    document.getElementById('statsLabel').textContent = `Showing ${startIndex + 1} through ${endIndex} out of ${totalFiltered}`;
    document.getElementById('pageBtnText').textContent = currentPage;
    document.getElementById('btnPrevPage').style.display = currentPage > 1 ? 'block' : 'none';
    document.getElementById('btnNextPage').style.display = currentPage < totalPages ? 'block' : 'none';
...
function buildCard(icon) {
    ...
    const collabStr = (m.isCollab && m.collabWith?.length) ? `<span class="font-pusab"> & </span><span class="font-golden">${m.collabWith.join(', ')}</span>` : '';
    ...
    const projectTag = m.hasProjectFiles ? `<span class="tag tag-project">Project Files</span>` : '';
    const projectBtn = m.hasProjectFiles ? `<button class="gd-btn btn-pink font-pusab project-btn" style="font-size: 1.4rem; padding: 6px 6px; margin-top: 10px;">Get Project</button>` : '';
    ...
    card.innerHTML = `
        ...
        <div class="card-info">
            ...
            <p class="card-author font-pusab">by <span class="font-golden">${escHtml(m.author)}</span>${collabStr}</p>
    ...
    card.querySelector('.download-btn').addEventListener('click', () => downloadIcon(icon));
    if (m.hasProjectFiles) {
        card.querySelector('.project-btn').addEventListener('click', () => downloadProjectFiles(icon));
    }
    return card;
}
...
async function setupIconImage(icon, card) {
    ...
    try {
        await renderer.renderIcon(icon.pngFile, icon.plistFile, icon.meta, getColorOpts());
...
function parseXMLPlist(xmlText) {
    ...
    function parseValue(element) {
        ...
        switch (tagName) {
            ...
            case 'array':
                const array = [];
                for (const child of element.children) array.push(parseValue(child));
                return array;
            case 'dict':
                const dict = {};
...
function generateXMLPlist(data) {
    function valueToXML(value, indent = 0) {
        ...
        if (typeof value === 'string') return `${spaces}<string>${value}</string>`;
        if (typeof value === 'number') return Number.isInteger(value) ? `${spaces}<integer>${value}</integer>` : `${spaces}<real>${value}</real>`;
        if (typeof value === 'boolean') return `${spaces}<${value}/>`;
        if (Array.isArray(value)) {
            if (value.length === 0) return `${spaces}<array/>`;
            const items = value.map(item => valueToXML(item, indent + 1)).join('\n');
            return `${spaces}<array>\n${items}\n${spaces}</array>`;
        }
        if (typeof value === 'object' && value !== null) {
            const entries = Object.entries(value);
            if (entries.length === 0) return `${spaces}<dict/>`;
            const items = entries.map(([k, v]) => `${spaces}\t<key>${k}</key>\n${valueToXML(v, indent + 1)}`).join('\n');
            return `${spaces}<dict>\n${items}\n${spaces}</dict>`;
        }
...
function packRectangles(frames, gap = 2) {
    const sortedFrames = [...frames].sort((a, b) => b.newSize[1] - a.newSize[1]);
    const bins = [];
    ...
    for (const frame of sortedFrames) {
        ...
        if (!placed) {
            const newY = bins.length > 0 ? Math.max(...bins.map(bin => bin.y + bin.maxHeight + gap)) : 0;
            const newBin = { x: 0, y: newY, width: frameWithGap, maxHeight: frameH };
            bins.push(newBin);
    ...
    const sheetHeight = bins.length > 0 ? Math.max(...bins.map(bin => bin.y + bin.maxHeight)) : 0;
...
async function portToHD(pngBlob, plistBlob) {
    try {
        ...
        for (const [frameName, frameData] of Object.entries(frames)) {
            ...
            const [offsetX, offsetY] = parseBracedTuple(frameData.spriteOffset || '{0,0}');
            ...
            if (isRotated) {
                ...
                finalCtx.drawImage(rotatedCanvas, -rotatedCanvas.width / 2, -rotatedCanvas.height / 2);
                ...
                spriteCtx.drawImage(canvas, x, y, spriteW, spriteH, 0, 0, spriteW, spriteH);
            }
            ...
            const preserveOffset = frameName && frameName.includes("GJ_table_side_001");
            const newOffset = preserveOffset ? [offsetX, offsetY] : [offsetX / 2, offsetY / 2];
        ...
        const { placements, sheetWidth, sheetHeight } = packRectangles(processedFrames, 2);
        ...
        processedFrames.forEach(frame => {
            ...
            newFrames[frame.name] = {
                ...
                textureRect: formatTextureRect(placement.x, placement.y, frame.newSize[0], frame.newSize[1]),
        ...
        if (metadata.realTextureFileName) {
            metadata.realTextureFileName = metadata.realTextureFileName.replace('-uhd', '-hd');
        }
        if (metadata.textureFileName) {
            metadata.textureFileName = metadata.textureFileName.replace('-uhd', '-hd');
        }
        ...
        return {
            plist: newPlistXml,
            png: await new Promise(resolve => newSheet.toBlob(resolve, 'image/png'))
        };
...
async function downloadIcon(icon) {
    ...
    const baseName = icon.pngFile.name.replace(/-uhd\.png$/i, '').replace(/\.png$/i, '');
...
async function downloadProjectFiles(icon) {
    const baseName = icon.meta.iconName.replace(/[^a-zA-Z0-9_\- ]/g, '').replace(/\s+/g, '_');
    try {
        ...
        if (!projZip) throw new Error("Project files missing from this archive!");
